...
import { adaptUserToClient, adaptCommentToClient } from '../utils/adapterToClient';
import { adaptUserToServer, adaptFilmToServer, adaptCommentToServer } from '../utils/adapterToServer';
...
export const addFilm = createAsyncThunk<Film, NewFilm, { extra: Extra }>(
  ...
  async (filmData, { extra }) => {
    const { api } = extra;
    const { data } = await api.post<Film>(APIRoute.Films, adaptFilmToServer(filmData));
...
>(`${NameSpace.Reviews}/postReview`, async ({ id, review }, { extra }) => {
  const { api } = extra;
  const { data } = await api.post<Review>(`${APIRoute.Comments}/${id}`, adaptCommentToServer(review, id));
